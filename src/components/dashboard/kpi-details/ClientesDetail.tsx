      ...
      {novos.length === 0 ? (
        ...
        <div className="space-y-1">
          {novos.map((c) => (
            <div
              key={c.id}
              className="p-3 rounded-lg hover:bg-slate-50 transition-colors flex items-center gap-3"
            >
              <div className="w-8 h-8 rounded-full bg-slate-100 flex items-center justify-center flex-shrink-0">
                {c.tipo_pessoa === 'pj' ? (
                  <Building2 className="w-3.5 h-3.5 text-[#46627f]" />
                ) : (
                  <User className="w-3.5 h-3.5 text-[#46627f]" />
                )}
              </div>
