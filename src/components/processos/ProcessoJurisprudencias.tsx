...
export default function ProcessoJurisprudencias({ processoId }: ProcessoJurisprudenciasProps) {
  ...
  const getBadgeRelevancia = (relevancia: string) => {
    const styles = {
      alta: 'bg-emerald-100 text-emerald-700 border-emerald-200 dark:bg-emerald-500/10 dark:text-emerald-400 dark:border-emerald-700',
      media: 'bg-amber-100 text-amber-700 border-amber-200 dark:bg-amber-500/10 dark:text-amber-400 dark:border-amber-700',
      baixa: 'bg-slate-100 text-slate-700 border-slate-200 dark:bg-surface-2 dark:text-slate-300 dark:border-slate-700'
    }
        ...
        <CardContent className="p-4">
          <div className="flex items-center gap-2">
            ...
            <input
              ...
              className="flex-1 px-4 py-2 border border-slate-200 dark:border-slate-700 rounded-lg text-sm bg-white dark:bg-surface-1 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-[#89bcbe]"
            />
          </div>
        </CardContent>
      </Card>
      ...
      {jurisprudencias.length === 0 ? (
        <Card className="border-slate-200 dark:border-slate-700 shadow-sm">
          <CardContent className="py-12 text-center">
            ...
            <p className="text-sm font-medium text-slate-600 dark:text-slate-400 mb-1">Nenhuma jurisprudência cadastrada</p>
        ...
        <Card className="border-slate-200 dark:border-slate-700 shadow-sm">
          <CardContent className="py-8 text-center">
            <p className="text-sm text-slate-500 dark:text-slate-400">Nenhuma jurisprudência encontrada para "{searchTerm}"</p>
          </CardContent>
        </Card>
      ) : (
        ...
        <div className="space-y-4">
          {jurisprudenciasFiltradas.map(jurisp => (
            <Card key={jurisp.id} className="border-slate-200 dark:border-slate-700 shadow-sm hover:shadow-lg transition-shadow">
              <CardHeader className="pb-3">
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1">
                    ...
                    <h4 className="text-sm font-bold text-[#34495e] dark:text-slate-200 mb-1">
                      {jurisp.tribunal}
              ...
              <CardContent className="space-y-3">
                <div>
                  <p className="text-xs font-semibold text-[#46627f] dark:text-slate-400 mb-1">EMENTA:</p>
                  <p className="text-sm text-slate-700 dark:text-slate-300 leading-relaxed">{jurisp.ementa}</p>
                </div>

                {jurisp.teses_aplicadas && jurisp.teses_aplicadas.length > 0 && (
                  <div>
                    <p className="text-xs font-semibold text-[#46627f] dark:text-slate-400 mb-2">Teses Aplicadas:</p>
