...
import { Check, ChevronDown, Folder, Eye, EyeOff, Loader2, Plus } from 'lucide-react'
...
import {
  ANDAMENTO_TIPOS,
  TIPOS_ANDAMENTO_MANUAL,
  type AndamentoTipo,
} from '@/lib/constants/andamento-tipos'

interface RegistrarAndamentoModalProps {
  open: boolean
...
function TipoDropdown({
  ...
  const sel = ANDAMENTO_TIPOS[value]
    ...
    <div ref={ref} className="relative">
      ...
      {open && (
        <div className="absolute top-[calc(100%+6px)] left-0 right-0 z-30 bg-white dark:bg-surface-1 border border-[#e6e3da] dark:border-[#253345] rounded-xl shadow-[0_18px_40px_-12px_rgba(15,23,42,0.35)] p-1.5 max-h-[296px] overflow-y-auto">
          {TIPOS_ANDAMENTO_MANUAL.map((tipo) => {
            const o = ANDAMENTO_TIPOS[tipo]
...
export default function RegistrarAndamentoModal({
  open,
  onOpenChange,
  processoId,
  escritorioId,
  ...
  area,
  onSuccess,
}: RegistrarAndamentoModalProps) {
  const supabase = createClient()
  const [tipo, setTipo] = useState<AndamentoTipo>('acompanhamento_processual')
  ...
  useEffect(() => {
    if (open) {
      setTipo('acompanhamento_processual')
  ...
  const sel = ANDAMENTO_TIPOS[tipo]
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const { error } = await supabase.from('processos_movimentacoes').insert({
      ...
      if (continuar) {
        // mantém aberto, limpa só a descrição para o próximo
        setDescricao('')
      } else {
        onOpenChange(false)
      }
      ...
      <DialogContent className="sm:max-w-[600px] p-0 gap-0 overflow-visible">
        <DialogHeader className="px-6 pt-6 pb-5 border-b border-[#f0ede3] dark:border-[#1d2a3c]">
          ...
          <DialogDescription className="text-[12.5px] text-[#9aa1a8] dark:text-slate-400">
            Adicione um andamento manual à timeline do processo.
          </DialogDescription>
        </DialogHeader>
          ...
          <div className="flex items-center gap-2.5 px-3 py-2.5 mb-5 rounded-[10px] bg-[#faf8f2] dark:bg-[#0f141c] border border-[#f0ede3] dark:border-[#1d2a3c]">
            ...
            <div className="min-w-0">
              <div className="text-[12.5px] font-semibold text-[#2c3e50] dark:text-slate-200 truncate">
                {clienteNome || 'Processo'} <span className="italic text-[#9aa1a8] font-normal">v.</span> {parteContraria || '—'}
              </div>
              <div className="text-[11px] text-[#9aa1a8] dark:text-slate-500 font-mono mt-0.5">
                {numeroPasta}{area ? ` · ${area}` : ''}
              </div>
            </div>
          </div>
