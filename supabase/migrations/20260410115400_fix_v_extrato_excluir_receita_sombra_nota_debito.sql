-- Corrige duplicação de Notas de Débito no extrato financeiro.
--
-- Cada Nota de Débito gera dois registros: o documento (financeiro_notas_debito)
-- e a receita-sombra (financeiro_receitas com categoria='custas_reembolsadas').
-- A view v_extrato_financeiro estava listando ambos no UNION, fazendo a mesma ND
-- aparecer duas vezes na tela de Receitas e Despesas. O mesmo padrão já era tratado
-- corretamente para faturas (filtro por r.fatura_id) e levantamentos (NOT EXISTS),
-- mas faltava o equivalente para notas de débito.
--
-- Esta migration redefine a view adicionando o filtro NOT EXISTS para esconder
-- receitas-sombra cuja ND vinculada está ativa (status NOT IN rascunho/cancelada).
...
CREATE OR REPLACE VIEW v_extrato_financeiro AS
 SELECT r.id,
  ...
  WHERE r.status <> 'cancelado'::receita_status_enum
    AND (r.fatura_id IS NULL OR (EXISTS ( SELECT 1
           FROM financeiro_faturamento_faturas ff
    ...
    AND NOT (EXISTS ( SELECT 1
           FROM financeiro_levantamentos fl
          WHERE fl.receita_id = r.id AND fl.status <> 'cancelado'::text))
    AND NOT (EXISTS ( SELECT 1
           FROM financeiro_notas_debito nd
          WHERE nd.receita_id = r.id AND nd.status NOT IN ('rascunho'::nota_debito_status, 'cancelada'::nota_debito_status)))
UNION ALL
 SELECT f.id,
    f.escritorio_id,
    'receita'::text AS tipo_movimento,
