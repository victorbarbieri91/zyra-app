...
interface UseConsultaDocumentosReturn {
  documentos: ConsultaDocumento[]
  total: number
  loading: boolean
  uploading: boolean
  uploadProgress: number
  uploadDocumentos: (files: File[]) => Promise<void>
  getSignedUrl: (documento: ConsultaDocumento) => Promise<string | null>
  deleteDocumento: (documento: ConsultaDocumento) => Promise<boolean>
  reload: () => Promise<void>
}
...
export function useConsultaDocumentos(consultaId: string): UseConsultaDocumentosReturn {
  const supabase = createClient()
  const { escritorioAtivo } = useEscritorioAtivo()
  const [documentos, setDocumentos] = useState<ConsultaDocumento[]>([])
  const [loading, setLoading] = useState(true)
  const [uploading, setUploading] = useState(false)
  const [uploadProgress, setUploadProgress] = useState(0)

  const load = useCallback(async () => {
    if (!consultaId || !escritorioAtivo) {
      setDocumentos([])
      setLoading(false)
      return
    }
    ...
    try {
      ...
      const docs: ConsultaDocumento[] = (data || []).map((doc: Record<string, unknown>) => {
  ...
  }, [consultaId, escritorioAtivo, supabase])
  ...
  const uploadDocumentos = useCallback(async (files: File[]) => {
    if (!consultaId || !escritorioAtivo || files.length === 0) return
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      try {
        ...
        const storagePath = `${escritorioAtivo}/${consultaId}/${uniqueId}-${safeName}`
        ...
        const { error: movError } = await supabase.from('consultivo_movimentacoes').insert({
          consulta_id: consultaId,
          escritorio_id: escritorioAtivo,
  ...
  }, [consultaId, escritorioAtivo, supabase, load])

  const getSignedUrl = useCallback(async (documento: ConsultaDocumento): Promise<string | null> => {
  ...
  const deleteDocumento = useCallback(async (documento: ConsultaDocumento): Promise<boolean> => {
    try {
      if (documento.storage_path) {
