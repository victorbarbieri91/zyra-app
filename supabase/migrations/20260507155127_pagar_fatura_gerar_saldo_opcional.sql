-- Adiciona parâmetro p_gerar_saldo a pagar_fatura.
-- Quando p_gerar_saldo = false e há diferença entre o esperado e o recebido,
-- a fatura é marcada como 'paga' (e não 'parcialmente_paga') sem criar receita
-- filha de saldo. O `valor_total` da fatura e o `valor` da receita-shadow são
-- preservados (Opção A — perdão silencioso da diferença); apenas `valor_pago`
-- reflete o que efetivamente entrou.
...
BEGIN
  ...
  -- v_finalizar = true quando a cobrança deve ser encerrada (pagamento total OU perdão da diferença).
  -- Excedente (v_saldo_restante < -0.01) também finaliza, já coberto por (<= 0.01).
  v_finalizar := (v_saldo_restante <= 0.01) OR (NOT p_gerar_saldo);
  ...
  SET status = v_novo_status,
      valor_pago = CASE
        -- Pago total: o que entrou bate com o esperado.
        WHEN v_saldo_restante <= 0.01 THEN v_total_esperado
        -- Perdão: registra apenas o que efetivamente entrou.
  ...
  IF v_receita_existente_id IS NOT NULL THEN
    IF v_finalizar THEN
      -- Pagamento total OU perdão: marca como pago.
      -- valor_pago: total = valor_liquido (zera arredondamentos); perdão = o que entrou de fato.
      UPDATE financeiro_receitas
      SET status = 'pago',
          valor_pago = CASE
            WHEN v_saldo_restante <= 0.01 THEN COALESCE(valor_liquido, valor)
            ELSE COALESCE(valor_pago, 0) + p_valor_pago
          END,
    ...
    ELSE
      -- Pagamento parcial COM cobrança da diferença (comportamento original).
      UPDATE financeiro_receitas
      SET status = 'pago',
  ...
  ELSE
    ...
    -- valor: quando finalizar (total ou perdão) → bruto da fatura (preserva contratado).
    --        quando vai gerar saldo → recebido (a filha terá o restante).
    -- valor_pago: o que entrou de fato.
    INSERT INTO financeiro_receitas (
      ...
      'honorarios',
      CASE WHEN v_finalizar THEN v_fatura.valor_total ELSE p_valor_pago END,
    ...
    IF v_saldo_restante > 0.01 AND p_gerar_saldo THEN
      INSERT INTO financeiro_receitas (
        escritorio_id, tipo, cliente_id, fatura_id,
        descricao, categoria, valor, data_competencia, data_vencimento,
        status, valor_pago, observacoes, created_by, updated_by
